...
class AudioPlayer {
  constructor() {
    ...
    this.songs = [
      "203932__aptbr__lofi-guitar.wav",
      ...
      "Juanitos - Exotica.mp3",
      "arsonist - Hot salsa trip.mp3",
    ];
  ...
  play() {
    if (this.dancer) this.dancer.pause();
...
class SoundBar {
  ...
  getAOFValue(index, count, offset = 0) {
    ...
    for (var i = 0; i < this.values.length; i++) {
      if ((i + offset) % (count * 3) === index) {
        sum += Math.min(1, (this.smoothValues[i] * 0.5) / this.smoothAvg);

        num += 1;
      }
    }
    if (num > 1) return (1.1 * sum) / num;
    return 0;
  }
  getPct(pct) {
    let sum = 0;
    for (var i = 0; i < this.values.length; i++) {
      ...
      if (d > 0.9) sum += d ** 5 * 0.14 * this.smoothValues[i];
    }
  ...
  update(t, dt, frame) {
    ...
    for (var i = 0; i < this.values.length; i++) {
      ...
      this.values[i] *= (0.03 + pct) * 80;
      this.values[i] = Math.min(1, this.values[i]);
      // this.values[i] *=  10

      this.smoothValues[i] =
        this.smoothValues[i] * this.smoothing +
        this.values[i] * (1 - this.smoothing);
    ...
    this.smoothAvg = this.smoothAvg * 0.95 + 0.05 * this.avg;
  }
  ...
  draw(p) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      let val = Math.min(1, (this.smoothValues[i] * 0.5) / this.smoothAvg);
