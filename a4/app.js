...
let canvasBackground = [0,0,0,1] // Adjust the background color if you want
let curvePoints = []
let isPaused = false
...
let DEBUG_DRAW = {
	"BasicParticleSystem": false,
	"BugsParticleSystem": true,
	"BoidsParticleSystem": true,
	"SpringyParticleSystem": true
}
...
let systemClasses = [
	FoodParticleSystem,
	DungBeetleParticleSystem,
	PredatorParticleSystem,
]
		...
		heatmap2 = new p5(
			...
			(p) => {
				...
				p.draw = () => {
					...
					systems.forEach(sys => {
						if (sys.drawHeatmap2)
							sys.drawHeatmap2(p, heatmapScale)
					})
					p.pop()
				}
			},document.getElementById("heatmap"))	
...
function readHeatmapAt2(pt) {
	let x = pt[0]/heatmapScale
	let y = pt[1]/heatmapScale
	
	let val = heatmap2.get(x, y)
	return val
}

function debugDrawHeatmap2(p) {
	let tileSize = 10
	let tileW = p.width/tileSize
	let tileH = p.height/tileSize
